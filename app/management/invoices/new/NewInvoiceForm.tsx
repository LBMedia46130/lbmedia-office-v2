...
export type NewInvoiceInitialData = {
  companyId?: string;
  date?: string;
  dueDate?: string;
...
type Props = {
  ...
  tax: TaxOption | null;
  items: ItemOption[];
  initialData?: NewInvoiceInitialData;
};
...
function createLineFromInitial(
  line: NonNullable<
    NewInvoiceInitialData["lines"]
  >[number],
  defaultTaxId = ""
): InvoiceLine {
...
export default function NewInvoiceForm({
  ...
  const [
    ...
    initialData?.lines?.length
      ? initialData.lines.map(
          (line) =>
            createLineFromInitial(
              line,
              tax?.tax_id ??
                ""
            )
  ...
  async function handleSubmit(
    ...
    if (invalidLine) {
      setErrorMessage(
        "Vérifie les lignes de la facture : prestation, quantité, prix HT et remise."
      );
            ...
            (
              ...
              return (
                ...
                >
                  ...
                  <div className="grid gap-4 xl:grid-cols-[1.8fr_2.5fr_0.7fr_1fr_1.25fr_0.9fr]">
