...
export default function AuditHistoryPage() {
  ...
  async function loadAudits(targetPage = page, targetSearch = search, targetStatus = status) {
    setLoading(true);
    setError(null);
    try {
      ...
      const response = await fetch(`/api/audit/history?${params.toString()}`, { cache: "no-store" });
  ...
  async function updateStatus(auditId: string, nextStatus: AuditStatus) {
    setError(null);
    try {
      ...
      setAudits((current) => current.map((audit) => audit.id === auditId ? { ...audit, status: nextStatus } : audit));
    } catch (updateError) {
      setError(updateError instanceof Error ? updateError.message : "Impossible de modifier le statut.");
    }
  }
  async function deleteAudit(audit: Audit) {
    if (!window.confirm(`Supprimer définitivement l’audit de ${audit.website_url} ?`)) {
      return;
    }
    setError(null);
    try {
      const response = await fetch(`/api/audit/${audit.id}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message ?? "Impossible de supprimer l’audit.");
      }
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">Historique des audits</h1>
            <p className="mt-2 text-sm text-slate-600">Retrouver, filtrer et consulter tous les audits enregistrés.</p>
          </div>
          <Link href="/audit" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">← Retour à l’audit</Link>
        </div>
        <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
            <form onSubmit={handleSearch} className="flex flex-1 gap-2">
              <div className="flex-1">
                <label className="mb-1 block text-sm font-semibold text-slate-700">Entreprise ou site</label>
                <input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Rechercher une entreprise ou une URL..." className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
              </div>
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
            <p className="text-sm text-slate-500">{pagination.total} audit{pagination.total > 1 ? "s" : ""} enregistré{pagination.total > 1 ? "s" : ""}</p>
          ...
          {loading ? <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 px-4 py-10 text-center text-sm text-slate-500">Chargement des audits...</div> : null}
          {!loading && audits.length === 0 ? <div className="mt-6 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center"><p className="font-medium text-slate-700">Aucun audit trouvé</p><p className="mt-1 text-sm text-slate-500">Modifiez la recherche ou les filtres.</p></div> : null}
                ...
                <table className="min-w-full divide-y divide-slate-200">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Site</th>
                      <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-500">Score</th>
                    ...
                    {audits.map((audit) => (
                      <tr key={audit.id} className="align-top">
                        ...
                        <td className="px-4 py-4">
                          <select value={audit.status} onChange={(event) => void updateStatus(audit.id, event.target.value as AuditStatus)} className="rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700">
                            {(Object.keys(statusLabels) as AuditStatus[]).map((value) => <option key={value} value={value}>{statusLabels[value]}</option>)}
                          </select>
                        </td>
                        <td className="px-4 py-4">
                          <div className="flex justify-end gap-2">
                            <Link href={`/audits/${audit.id}`} className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 transition hover:bg-blue-100">Voir l’audit</Link>
                            <button type="button" onClick={() => void deleteAudit(audit)} className="rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-100">Supprimer</button>
                          </div>
                        </td>
                      </tr>
                    ))}
