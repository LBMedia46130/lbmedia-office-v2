...
import { getWebsiteAuditById } from "@/lib/website-audits";
...
export default async function AuditHistoryDetailPage({
  ...
  const audit = await getWebsiteAuditById(auditId);
  ...
  if (audit.company_id) {
    const company = await getCompanyById(
      String(audit.company_id)
    );

    if (company) {
      redirect(
        `/companies/${company.id}/audits/${audit.id}`
      );
    }
  }
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-6xl px-6 py-10">
        <PageBanner
          ...
          title={getWebsiteLabel(audit.website_url)}
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              href={`/audit?url=${encodeURIComponent(
                audit.website_url
              )}`}
        ...
        <section className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-5">
          <p className="text-sm font-semibold text-amber-900">
            Cet audit n’est plus rattaché à une entreprise existante dans
            LBMedia Office.
          </p>

          <p className="mt-1 text-sm leading-6 text-amber-700">
            Il reste consultable depuis l’historique, mais les actions liées
            à une fiche entreprise ne sont pas disponibles.
          </p>
        </section>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-blue-500">
                Grille de notation {audit.scoring_version}
              </p>
            </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <AuditList
            eyebrow="Diagnostic"
            title="Points forts"
            items={audit.strengths}
          />

          <AuditList
            eyebrow="Diagnostic"
            title="Points à améliorer"
            items={audit.weaknesses}
          />
        </div>
